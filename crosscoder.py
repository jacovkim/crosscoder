...
class TopK(nn.Module):
    ...
    def forward(self, x):
        ...
        # scatter topk values to their positions
        output.scatter_(self.dim, indices, values)
...
class CrossCoder(nn.Module):
    def __init__(self, cfg):
        ...
        feature_init_sparsity = 0.05
        self.topK = TopK(k=int(cfg["dict_size"] * feature_init_sparsity))
    ...
    def get_losses(self, x):
        ...
        # aux loss
        # TODO: only if dead features more than k_aux_topk
        # topk_values, _ = torch.topk(acts, self.cfg.k_aux_topk, dim=-1)
        # aux_loss = (1 / topk_values.abs()).mean()
        ...
        # l1 loss -> sparsity regularization
        decoder_norms = self.W_dec.norm(dim=-1) # decoder_norms: [d_hidden, n_models]
